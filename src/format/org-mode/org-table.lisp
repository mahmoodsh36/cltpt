  ...
  (multiple-value-bind (line-start line-end)
    ...
    (let* ((trimmed-line-start
      ...
      (let ((reversed-children (nreverse row-children)))
        ...
        (values row-match (if (< line-end (cltpt/reader:reader-buffer-fill reader))
                              (1+ line-end)
                              line-end))))))
  ...
  (let* ((table-match (cltpt/combinator:make-match
    ...
    (loop
      ...
      (multiple-value-bind (line-start line-end)
        ...
        (let ((next-line-start (if (< line-end (cltpt/reader:reader-buffer-fill reader))
                                   (1+ line-end)
                                   line-end)))
          ...
          (setf current-pos next-line-start)
...
(defun table-match-to-list (str table-match &optional (include-hrules-p t))
  "takes an org-table match and returns a nested list representing the table's data.
ignores the new table-row-separator nodes."
  (let ((table-data))
...
(defun list-to-table-string (table-data)
  "takes a nested list (as generated by table-match-to-list)
