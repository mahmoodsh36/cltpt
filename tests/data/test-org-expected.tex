...
\\
  my first block
\\
  some text  \begin{subtheorem}
\\
    some text that will be included on export  \end{subtheorem}\end{theorem}
\\
...
\begin{theorem}
\\
then add the first number from the next list to get BROKENmultiply that by 5 to get BROKEN. anything is possible! this is only a simple example
\\
\end{theorem}
\\
\begin{itemize}
...
\\
\begin{lstlisting}import matplotlib
matplotlib.use('Agg')
