...
#+begin_src latex :file (cached-file "P9jJoKl.svg") :cache yes :eval no
  \begin{alg}
...
# in the following text removing the space before the "2[ex]" snippet causes issues with escaping because it thinks we're trying to escape starting a display math env
\[
...
* TODO header my secondary header :tag1:tag2:important:
...
* DONE completed task :test:
SCHEDULED: <2025-07-29 Tue 09:00:00>

* TODO repeating task with last repeat :test:
<2025-07-23 Wed 10:00:00 +1d>
:PROPERTIES:
:LAST_REPEAT: [2025-07-28 Mon 10:00:00]
:END:
...
code here
#+begin_src python :results output :eval no :exports both
  import requests
  print('whatever')
  print('whatever2')
#+end_src
...
#+begin_src python :results file :var filepath=(cached-file "WRB4q2d.svg") :exports both :eval no
  import matplotlib
  matplotlib.use('Agg')
